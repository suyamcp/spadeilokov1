...
export default function AboutSection({ data }: AboutSectionProps) {
  ...
  const desc2 = data.desc2 || 'This is a placeholder for your mountain story paragraph 2. You can describe your Overlook Cafe, the weather, safety precautions, commutes, and general atmosphere here.';
  ...
  const highlights = [
    {
      ...
      title: data.title ? 'Sea of Clouds' : 'Dynamic highlight 1',
    ...
    },
    {
      ...
      title: data.title ? 'Crisp Alpine Breeze' : 'Dynamic highlight 2',
      description: data.title ? 'Frequently cooler than Baguio center, enjoy climate averages of 12°C to 17°C with misty mountain smells.' : 'Configure custom highlight title & details in the Admin dashboard.'
    },
    {
      icon: <Compass className="w-5 h-5 text-gold-400" />,
      title: data.title ? 'Pristine Ridges' : 'Dynamic highlight 3',
      description: data.title ? 'Located safely along the pine cliff coordinates of Tuba, Benguet—guaranteeing 360 panoramic horizons.' : 'Configure custom highlight title & details in the Admin dashboard.'
    },
    {
      icon: <Sunrise className="w-5 h-5 text-gold-400" />,
      title: data.title ? 'Bonfires & Sunrises' : 'Dynamic highlight 4',
      description: data.title ? 'Unwind at twilight around open firepits roaring with dry pine wood logs, roasting sweet marshmallows.' : 'Configure custom highlight title & details in the Admin dashboard.'
    }
  ];
